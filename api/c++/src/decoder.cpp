...
SOFTLOQ_JSON_API const Element* Decoder::decodeJSON(const std::string& json_string) {
    ...
    if(!parseElement(json_segment, json_element)) return nullptr;
...
SOFTLOQ_JSON_API const Object* Decoder::decodeObject(const std::string& json_string) {
    ...
    if(!(parseWS(json_segment) && parseObject(json_segment, json_object) && parseWS(json_segment))) return nullptr;
...
SOFTLOQ_JSON_API const Array* Decoder::decodeArray(const std::string& json_string) {
    ...
    if(!(parseWS(json_segment) && parseArray(json_segment, json_array) && parseWS(json_segment))) return nullptr;
...
SOFTLOQ_JSON_API const String* Decoder::decodeString(const std::string& json_string) {
    ...
    if(!(parseWS(json_segment) && parseString(json_segment, json_string_element) && parseWS(json_segment))) return nullptr;
...
SOFTLOQ_JSON_API const Number* Decoder::decodeNumber(const std::string& json_string) {
    ...
    if(!(parseWS(json_segment) && parseNumber(json_segment, json_number) && parseWS(json_segment))) return nullptr;
...
SOFTLOQ_JSON_API const Bool* Decoder::decodeBool(const std::string& json_string) {
    ...
    if(!(parseWS(json_segment) && parseBool(json_segment, json_bool) && parseWS(json_segment))) return nullptr;
...
SOFTLOQ_JSON_API const Null* Decoder::decodeNull(const std::string& json_string) {
    ...
    if(!(parseWS(json_segment) && parseNull(json_segment, json_null) && parseWS(json_segment))) return nullptr;
...
SOFTLOQ_JSON_API const bool Decoder::parseValue(std::string_view& json_segment, std::unique_ptr<Element>& json_value) {
    return parseObject(json_segment, json_value)
        || parseArray(json_segment, json_value)
        || parseString(json_segment, json_value)
        || parseNumber(json_segment, json_value)
        || parseBool(json_segment, json_value)
        || parseNull(json_segment, json_value);
}
SOFTLOQ_JSON_API const bool Decoder::parseObject(std::string_view& json_segment, std::unique_ptr<Element>& json_object) {
    ...
    if(parseMatch(json_segment, "{") && parseMembers(json_segment, json_object) && parseMatch(json_segment, "}")) return true;
...
SOFTLOQ_JSON_API const bool Decoder::parseMembers(std::string_view& json_segment, std::unique_ptr<Element>& json_object) {
    ...
    while(parseMember(json_segment, key, value)) {
        ...
        if(!parseMatch(json_segment, ",")) break;
    }
    if(!json_object->as<Object>()->empty()) return true;
...
SOFTLOQ_JSON_API const bool Decoder::parseMember(std::string_view& json_segment, std::unique_ptr<Element>& key, std::unique_ptr<Element>& json_value) {
    ...
    if(parseWS(json_segment) && parseString(json_segment, key) && parseWS(json_segment) && parseMatch(json_segment, ":") && parseElement(json_segment, json_value)) return true;
...
SOFTLOQ_JSON_API const bool Decoder::parseArray(std::string_view& json_segment, std::unique_ptr<Element>& json_array) {
    ...
    json_segment = base_segment;
    if(parseMatch(json_segment, "[") && parseElements(json_segment, json_array) && parseMatch(json_segment, "]")) return true;
...
SOFTLOQ_JSON_API const bool Decoder::parseElements(std::string_view& json_segment, std::unique_ptr<Element>& json_array) {
    ...
    if(!json_array->as<Array>()->empty()) return true;
...
SOFTLOQ_JSON_API const bool Decoder::parseElement(std::string_view& json_segment, std::unique_ptr<Element>& json_element) {
    ...
    if(parseWS(json_segment) && parseValue(json_segment, json_element) && parseWS(json_segment)) return true;
...
SOFTLOQ_JSON_API const bool Decoder::parseStringCharacters(std::string_view& json_segment, std::string& json_characters) {
    ...
    while(!issue && !json_segment.empty()) {
        ...
        if(!character::convertUTF8ToCode(json_segment, utf8_codepoint, byte_count)) break;
        ...
        case '\\': {
            ...
            if(!parseStringEscape(json_segment, json_characters)) issue = true;
            break;
        }

        default:
            if(character::convertCodeToUTF8(utf8_codepoint, json_characters))
...
SOFTLOQ_JSON_API const bool Decoder::parseStringEscape(std::string_view& json_segment, std::string& json_characters) {
    if(json_segment.empty()) return false;
    ...
    case 'u': {
        ...
        if(hex_segment.length() < 4) return false;
        if(!(isxdigit(hex_segment[0]) && isxdigit(hex_segment[1]) && isxdigit(hex_segment[2]) && isxdigit(hex_segment[3]))) return false;
        uint32_t accum = 0;
        for(int i = 0; i < 4; ++i) {
            accum *= 16;
            if('0' <= hex_segment[i] && hex_segment[i] <= '9') accum += static_cast<uint8_t>(hex_segment[i] - '0');
            else if('a' <= hex_segment[i] && hex_segment[i] <= 'f') accum += static_cast<uint8_t>(hex_segment[i] - 'a' + 10);
            else if('A' <= hex_segment[i] && hex_segment[i] <= 'F') accum += static_cast<uint8_t>(hex_segment[i] - 'A' + 10);
        }
        if(!character::convertCodeToUTF8(static_cast<char32_t>(accum), json_characters)) return false;
